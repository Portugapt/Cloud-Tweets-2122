...
def _create_app():
    print('INFO: functions.list_tweet_username.app.main._create_app')
...
def _formalize_request(request: Request,
                       default_username: str = 'No Username',
    ...
    argument_present = [
        bool(i) for i in [username, limit]]
...
def main(request):
    ...
    if len(results) > 0:
        ...
        response = app.response_class(
            response="No username found with this name",
            status=204
        )
    else:
        response = app.response_class(
