...
for csv_file in FILES_TO_PROCESS:
    ...
    csv_data = pd.read_csv(f"{BUCKET_LINK}/{csv_file}")
    #csv_data = (spark
    #            .read
    #            .format("csv")
    ...
    logging.info(f'START: Saving parsed Global data')
    #csv_file.printSchema()
    #BUCKET_LINK
    print(f'Saving to: {BUCKET_LINK}/parsed/{csv_file.split("/")[-1].split(".")[0]}.csv')
    
    csv_data.coalesce(1).write \
      .options(delimiter='|') \
      .csv(f'{BUCKET_LINK}/parsed/{csv_file.split("/")[-1].split(".")[0]}')
    logging.info(f'END: Saving parsed Global data')
